...
pub fn FaviconContainer<G: Html>() -> View<G> {
    view! {
        div(class="favicons") {
            a(class="favicon", 
                target="_blank", 
                href="https://www.github.com/jprugel", 
            ) {
                svg(
...
pub fn SettingsMenu<G: Html>() -> View<G> {
    ...
    let change_theme = move |_| {
        match theme.get() {
            ...
            Themes::Frappe => theme.set(Themes::Latte),
        };
    };
    let dropdown_ctx = use_context::<Signal<DropdownContext>>();
    let mouse_is_here = use_context::<Signal<MouseIsWithinSettingsMenu>>();
    let login_mouse_present = use_context::<Signal<LoginMP>>();
    let ctx = use_context::<Signal<LoginVisibility>>();
    let login_vis = move |_| {
        login_mouse_present.set(LoginMP(true));
        ctx.set(LoginVisibility(true));
    };
    view! {
        div(
